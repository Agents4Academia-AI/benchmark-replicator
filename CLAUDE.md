...
**Baseline Replicator** turns arXiv papers into clean, minimal, CPU-runnable baseline implementations via a deterministic pipeline of Claude Agent SDK sub-agents. State is shared only through files in the generated repo; the orchestrator controls flow—the model never auto-delegates.

## Pipeline Phases

1. **Planner** (opus): Reads PDF → writes `PLAN.md` with a plan for the most informative
   CPU-feasible experiment (within roughly tens of minutes to about an hour on a modern CPU),
   plus `.replicator/criteria.json` — the mechanizable criteria as stable ids + comparison + threshold.
...
uv run replicate https://arxiv.org/abs/XXXX.XXXXX             # full pipeline
...
Generated baselines target the paper's most informative CPU-feasible experiment: one main method, a real dataset or task from the paper (or a smaller version within budget), with the default run completing in roughly tens of minutes to about an hour on a modern CPU.
