...
- Preserve the plan's existing section structure and its guiding philosophy: a **clean, minimal,
  CPU-runnable** reference implementation of the paper's *one main method*, with a **cheap smoke
  run** (minutes, no large downloads) and measurable success criteria. Don't let revisions balloon
  the scope unless the user explicitly asks.
...
- If a request is unclear or in tension with the minimal/CPU-only philosophy, say so and ask rather
  than guessing.
...
{
  "criteria": [
    { "id": "loss_drop_pct", "metric": "training loss reduction over the smoke run, as a percentage",
