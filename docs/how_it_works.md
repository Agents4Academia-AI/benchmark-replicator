...
Python orchestrator** that runs a fixed sequence of scoped Codex agents. The key
...
```
                          ...
                          └──────────────────────────────────┘
                                          ...
                                          ▼
                            ╔═══════════════════════════════╗
            paper.pdf ─────▶║     ① PLANNER     (Codex)     ║─────▶ PLAN.md
            ...
            paper.txt ─────▶║   within budget (CPU or --gpu)║       + artifacts.json
                            ...
                            └─────────────────────────────────────────┘
                                          ...
                                          ▼
        ╭───────────────────────────────────────────────────────────────────────╮
        │  ③ CODER  (Codex): N sequential phases parsed from PLAN.md  (max 4)   │
        │                                                                       │
        │   for each Phase n of N (from the plan's Implementation Phases):      │
        │     ╔════════════════════════════╗                                    │
        │     ║   ③ CODER  (Codex)          ║  implements that phase's scope    │
        ...
        │                                                                       │
        │     ╔═══════════════════════╗      verdict.json                       │
        │     ║  ④a TESTER  (Codex)   ║─────▶ pass / fail ──┐                   │
        │     ╚═══════════════════════╝   (writes test suite)│                  │
        │                │ PASS                               │ FAIL            │
        │                ▼                                    │                 │
        │     ╔═══════════════════════╗                       │                 │
        │     ║④b BENCHMARKER (Codex) ║──▶ verdict.json       │                 │
        │     ╚═══════════════════════╝   (writes REPORT.md)  │                 │
        │                │                                    │                 │
        │                ▼                                    │                 │
        │     ┌───────────────────────────────┐               │                 │
        ...
        ╰────────────────┼──────────────────────────────────────────────────────╯
                         ▼
            ╔═════════════════════════╗
            ║   ⑤ CLEANER  (Codex)    ║─────▶ tidy src/ + README.md
            ╚═════════════════════════╝       (ruff, re-run tests)
                         │
                         ▼
                    ✅  DONE
...
Every phase is one independent Codex SDK thread with a **fresh context**
...
- a scoped set of capabilities included in its developer instructions.

Codex supplies its native filesystem, shell, and web tools. Each thread uses the generated
repo as its working directory, runs in the workspace-write sandbox, denies permission
escalations, allows network access for dependency and dataset downloads, and does not load
...
Capabilities: repository reads, scoped file writes, and web search/fetch for context. It does
...
### ② Reviser (Codex) &mdash; only on `[c]hat`
Unlike the one-shot phases, this is a **stateful** Codex thread (`run_chat_agent`). The user
types successive revision requests; the agent remembers the
conversation and edits `PLAN.md` / `.replicator/criteria.json` in place. The first message
tells it where the paper lives. Typing
...
its measured value. It can read and write repository files and run shell commands.
...
1. **Clear `verdict.json`**, then run the **Tester (Codex)** &mdash; it writes a small, fast,
   deterministic pytest suite, runs it, and writes a structured verdict. If it fails, skip to
   repair.
2. Clear `verdict.json`, run the **Benchmarker (Codex)** &mdash; it runs the implementation
...
- **④c Repair (Codex)** &mdash; the *only* phase that edits source. It receives the recorded
